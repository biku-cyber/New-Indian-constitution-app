...
(function () {
  ...
  const ICONS = {
    ...
    parts: svg('<rect x="5" y="7" width="12" height="14" rx="1.4" stroke="currentColor" stroke-width="1.6"/><rect x="8" y="4" width="12" height="14" rx="1.4" stroke="currentColor" stroke-width="1.6" fill="var(--color-bg,#151312)"/>'),
    schedules: svg('<rect x="4.5" y="4" width="15" height="17" rx="1.6" stroke="currentColor" stroke-width="1.6"/><path d="M8 9.5l1.4 1.4L12 8.4M8 15l1.4 1.4L12 13.9" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/><path d="M14 9.5h3M14 15h3" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>'),
  ...
  // Signature element: a single-line stone arch, echoing "সেতু" (bridge) —
  // the bridge a reader crosses from a question to the text of the law.
  // Used once, quietly, behind the home title. Never repeated elsewhere.
  const BRIDGE_SVG = `<svg viewBox="0 0 640 92" fill="none" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M0 88 Q60 88 70 60 Q90 10 160 10 Q230 10 250 60 Q265 88 330 88 Q395 88 410 60 Q430 10 500 10 Q570 10 590 60 Q600 88 640 88" stroke="currentColor" stroke-width="1.4"/>
    <path d="M70 60 V88 M160 10 V88 M250 60 V88 M410 60 V88 M500 10 V88 M590 60 V88" stroke="currentColor" stroke-width="1"/>
  </svg>`;
  ...
  async function renderHome(root) {
    ...
    const bridgeMotif = el("div", { class: "bridge-motif", html: BRIDGE_SVG, "aria-hidden": "true" });
  ...
  async function renderPreamble(root) {
    const entry = await fetchJSON("data/constitution/preamble.json");
    const ctx = {
  ...
  async function renderList(root, moduleKey) {
    ...
    const raw = await fetchJSON(mod.dataUrl);
  ...
  async function renderCaseStudyList(root) {
    const data = await fetchJSON("data/casestudies/casestudies.json");
    const page = el("div", { class: "page" });
    const resultCount = el("div", { class: "result-count text-caption" });
  ...
  async function renderCaseStudyDetail(root, id) {
    ...
    const body = el("div", { class: "reader-body" }, Reader.renderContent(entry));
    const readerView = el("div", { class: "reader-view" }, [toolbar, body]);
    Theme.applyReaderTheme(readerView);
    mount(root, el("div", { class: "view" }, [readerView]));
  }
